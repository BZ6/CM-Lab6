...
std::vector<std::pair<double, double>>
DifferentialEquationCalculator::euler() const {
  std::vector<std::pair<double, double>> results;
  double x = x0;
  double y = y0;
  results.emplace_back(x, y);
  ...
  while (x < xn) {
    y += h * equation.evaluate(x, y); // Euler's method to find the approximate next value of y
    x += h;
...
std::vector<std::pair<double, double>> DifferentialEquationCalculator::solve() {
  ...
  case DifferentialEquationCalculator::RungeKutta4:
    return rungeKutta4();
  case DifferentialEquationCalculator::Euler:
    return euler();
